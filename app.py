...
if page=="📤 Upload Data":
    ...
    if df.empty:
        ...
        st.altair_chart(alt.Chart(bench_chart).mark_bar().encode(
            x='Bench_Status',y='Count',color='Bench_Status'), use_container_width=True)

        # Dept Util Chart
        dept_util = df.groupby('Dept')['True_Utilization'].mean().reset_index()
        st.altair_chart(alt.Chart(dept_util).mark_bar().encode(
            x='Dept',y='True_Utilization',color='Dept'), use_container_width=True)
    ...
    else: st.dataframe(df[['Employee','Dept','Bench_Status','True_Utilization']],height=400)
    ...
    if df_emp.empty or df_skills.empty: st.info("Upload both Employee Activity and Skills file first")
        ...
        def rec(s): 
            emp_skills = str(s).split(",") if pd.notnull(s) else []
    ...
    if df_emp.empty or df_proj.empty: st.info("Upload Employee Activity and Project Assignment file first")
    else:
        assignments = []
    ...
    if df.empty: st.info("Upload Employee Activity first")
        ...
        if 'Bench_Duration' in df.columns:
            scatter = alt.Chart(df).mark_circle(size=60).encode(
                x='Bench_Duration',y='True_Utilization',
                color='Bench_Status',
