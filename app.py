...
def ai_recommendations(df, role):
    ...
    if role=="HR Head":
        ...
        for _, row in low_util.iterrows():
            recs.append(f"Upskill {row['Employee']} in missing skills: improve billing.")
...
if page=="📤 Upload Data":
    ...
    role = st.selectbox("Select Role View:", ["HR Head", "Project Manager"])

    if df.empty:
        st.info("Upload Employee Activity first")
    else:
        # Filter if PM and reportees file uploaded
        ...
        # Line Chart - Utilization by Department
