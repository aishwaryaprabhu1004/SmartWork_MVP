...
page = st.sidebar.radio(
    "Select a Page",  # ✅ non-empty label for accessibility
    options=[
        "🏠 Dashboard",
        "🪑 Bench Utilization",
    ...
    label_visibility="collapsed"  # ✅ visually hidden, accessible to screen readers
)
...
# ---------------- Helper Functions ----------------
def load_file(file):
    """Reads uploaded file safely (handles CSV & Excel)"""
    try:
        if file:
...
def calculate_utilization(df):
    """AI-inspired heuristic to compute utilization"""
    if df.empty:
        return df
    df['Activity_Score'] = (
...
if page == "📤 Upload Data":
    ...
    st.info("Upload Employee Activity, Skill Training, and Project Assignment files (CSV/XLSX).")
    ...
    if df.empty:
        ...
        chart1 = alt.Chart(bench_chart).mark_bar().encode(
            x=alt.X('Bench_Status', sort=None),
            y='Count',
            color='Bench_Status'
        )
